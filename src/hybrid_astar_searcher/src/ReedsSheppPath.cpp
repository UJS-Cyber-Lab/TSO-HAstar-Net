...
namespace planning
{
  ...
  double NormalizeAngle(const double angle) {
    // Calculate the modulus (remainder) of angle + M_PI and 2.0 * M_PI. The purpose of this step is to map the angle to the range [0, 2pi].
    double a = std::fmod(angle + M_PI, 2.0 * M_PI);
    // This if statement handles the case where the angle after modulus is less than 0. This situation may occur when the input angle is negative and its absolute value is greater than π. In this case, we need to add 2π to the result to ensure it falls within the range [0, 2π].
    if (a < 0.0) {
      a += (2.0 * M_PI);
    }
    // Subtract M_PI from a to shift the angle to the range [-pi, pi).
    return a - M_PI;
  }

  // Convert a point (x, y) in Cartesian coordinates to polar coordinates (r, theta)
  ...
  // Generate the shortest Reed-Shepp (RS) curve
  bool ReedShepp::ShortestRSP(const std::shared_ptr<Node3d> start_node,
                              const std::shared_ptr<Node3d> end_node,
                              ReedSheppPath& optimal_path) {
    std::vector<ReedSheppPath> all_possible_paths;

    // Call the GenerateRSP function to calculate all possible Reeds-Shepp paths between the start node and the end node, and store them in the all_possible_paths vector.
    ...
    // Traverse all possible paths through a loop to find the shortest path. Specifically, use optimal_path_length and optimal_path_index to record the length and index of the current shortest path, respectively. During traversal, if the total length (total_length) of the current path is greater than zero and less than the current shortest path length plus a threshold (set to 3 here), update the shortest path length and index.
    ...
    for (size_t i = 0; i < paths_size; ++i) {
      if (all_possible_paths.at(i).total_length > 0 &&
        ...
        optimal_path_index = i; // Record the index of the current shortest path
        optimal_path_length = all_possible_paths.at(i).total_length; // Record the length of the current shortest path
      }
    }
    // Call the GenerateLocalConfigurations function to generate the local configurations (i.e., the positions and heading angles of the start and end points of each segment) of the optimal path, and store them in optimal_path.
    ...
    // Check if the endpoint of the optimal path matches the position and heading angle of the goal node.
    ...
    // Store the x, y, phi, gear, total_length, segs_types, and segs_lengths of the optimal path in optimal_path
  ...
  bool ReedShepp::GenerateRSP(const std::shared_ptr<Node3d> start_node,
                              const std::shared_ptr<Node3d> end_node,
                              std::vector<ReedSheppPath>* all_possible_paths) {
    // Calculate the differences (dx, dy, and dphi) between the start node and the end node in terms of x, y, and heading angle, as well as the cosine and sine values (c and s) of the start node.
    ...
    // By normalizing the starting point to (0,0,0), calculate the normalized target point's x and y coordinates. Here, the maximum curvature (max_kappa_) is used to scale the x and y values.
    double x = (c * dx + s * dy) * max_kappa_; 
    // max_kappa_ = reciprocal of the minimum turning radius
    double y = (-s * dx + c * dy) * max_kappa_;
    // Call the SCS, CSC, CCC, CCCC, CCSC, and CCSCC functions in sequence to generate different types of Reeds-Shepp paths and store them in the all_possible_paths vector. If the generation of a certain type of path fails, corresponding comments will be printed.
    // SCS: Represents driving straight (Straight) first, then turning into a curve (Curves), and finally driving straight again. This path consists of three elements: the length of the straight segment, the turning angle, and the length of the straight segment.
    if (!SCS(x, y, dphi, all_possible_paths)) {
    // std::cout << "Fail at SCS";
    }
    // CSC: Represents driving in a curve first, then turning into a straight line, and finally driving in a curve again. This path consists of three elements: the turning angle, the length of the straight segment, and the turning angle.
    if (!CSC(x, y, dphi, all_possible_paths)) {
      // std::cout << "Fail at CSC";
    }
    // CCC: Represents driving in three consecutive curves, with the turning directions of the first two curves opposite to each other, and the third curve having the same turning direction as the first two. This path consists of three elements: the curve radius, the curve radius, and the curve radius.
    if (!CCC(x, y, dphi, all_possible_paths)) {
      // std::cout << "Fail at CCC";
    }
    // CCCC: Represents driving in four consecutive curves, with the turning directions of every two curves opposite to each other. This path consists of four elements: the curve radius, the curve radius, the curve radius, and the curve radius.
    if (!CCCC(x, y, dphi, all_possible_paths)) {
      // std::cout << "Fail at CCCC";
    }
    // CCSC: Represents driving in three consecutive curves, with the turning directions of the first two curves opposite to each other, and the third segment being a straight line. This path consists of four elements: the curve radius, the curve radius, the length of the straight segment, and the curve radius.
    if (!CCSC(x, y, dphi, all_possible_paths)) {
      // std::cout << "Fail at CCSC";
    }
    // CCSCC: Represents driving in four consecutive curves, with the turning directions of every two curves opposite to each other, and the third segment being a straight line, while the fourth curve has the same turning direction as the first two. This path consists of five elements: the curve radius, the curve radius, the length of the straight segment, the curve radius, and the curve radius.
  ...
  // This function calculates the path for the robot to drive straight for a certain distance, then make a curve turn, and finally drive straight again to reach the destination. The calculation process uses two curve driving methods: SLS and SRS.
  bool ReedShepp::SCS(const double x, const double y, const double phi,
                      std::vector<ReedSheppPath>* all_possible_paths) {
    RSPParam SLS_param;
    // Call the SLS function to calculate the path parameters for the robot to travel from the starting point to the target point using the SLS method, including the turning angle and the straight driving distance. Then, pass these path parameters to the SetRSP function to generate the corresponding path and store it in the all_possible_paths vector.
    ...
    RSPParam SRS_param;
    // Calculate the path for the robot to travel in the opposite direction (i.e., along the -y direction), which is the SRS path. Similarly, pass the SRS path parameters to the SetRSP function to generate the path and store it in the all_possible_paths vector.
  ...
  // Calculate the path parameters for the robot to drive straight for a certain distance, then make a curve turn to the left, and finally drive straight again to reach the destination.
  void ReedShepp::SLS(const double x, const double y, const double phi,
                      RSPParam* param) {
    double phi_mod = NormalizeAngle(phi);
    // Convert phi to a value within the range [-pi, pi]
    double xd = 0.0;
    // Turning angle u, first straight driving distance t, curve driving distance v
    double u = 0.0;
    double t = 0.0;
    double v = 0.0;
    double epsilon = 1e-1; // 0.1 = 5 degrees
    // The robot turns left, check y
  ...
  bool ReedShepp::GenerateLocalConfigurations(
    ...
    pd = d;  
    // d means sample interval, pd means accumulated distance which has been passed
