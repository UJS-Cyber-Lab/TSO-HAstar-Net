...
namespace chibot::topologyplanning {
  ...
  void HalfStructPlanner::init(ros::NodeHandle &nh) {
    // 起终点最多生成1个额外点用于构成边，也就是一个接点，而且是离起终点最近的接点
    ...
    // ROS_INFO("connection_point_num:%d, connection_points_range:%f",connection_point_num, connection_points_range);
  ...
  void HalfStructPlanner::calculate_pre_graph(lines_type const& lines, vector<double> const& distance_table) {
    ...
    /* static_cast is a type conversion operator in C++. It is used to perform static type conversion, converting an expression to the specified type.
      ...
      In the given code, static_cast is used to convert the result of the expression traffic_routes_.size() * 2 + 2 + connection_point_num * 2 from a numeric type to an integer. This is because node_num_ is an integer variable, and the calculation result needs to be converted to an integer before assignment.
    ...
    // Number of start and end points in the road network (traffic_routes_.size()) * 2 (each start point is also an end point) + start and end points of the car (2) + new nodes introduced by the car's start and end points (connection_point_num * 2)
  ...
  void HalfStructPlanner::calculate_graph() {
    ...
    for (auto i = 0; i < connection_point_num; ++i) {
      geometry_msgs::PoseStamped p;
      p.header.frame_id = "map";
      // Store the start point and its connection points from the start set into nodes_. We only store the first few nodes (connection_point_num), discarding the rest. The first few nodes are those with smaller edge lengths (already sorted).
      if (i < start_set.size()) {
        ...
        // If the distance from the start point to the connection point is < connection_points_range (3), the first connection point (the one closest to the start point) must be updated, as there must be at least one edge from the start point to the connection point.
        // if (i < 1 || std::next(start_set.begin(), i)->nearest_dist < connection_points_range) {
    ...
    if (i < goal_set.size()) {
      ...
      // if (i < 1 || std::next(goal_set.begin(), i)->nearest_dist < connection_points_range) {
    ...
    for (auto i = 0; i < connection_point_num; ++i) {
      if (i < start_set.size()) {
        ...
        // if (std::next(start_set.begin(), i)->nearest_dist < connection_points_range) {

          for (auto j = 0; j < connection_point_num; ++j) {
          if (j < goal_set.size()) {
            auto gen = 2 + connection_point_num + j;
            // Index of the end point's connection point.
            // if (std::next(goal_set.begin(), j)->nearest_dist < connection_points_range) {
